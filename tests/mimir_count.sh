...
$JF mcount -t $nCPUs -o ${pref}_m15_s2M.jf -s 2M -C -m 15 seq10m.fa
$JF histo ${pref}_m15_s2M.jf.1.0 > ${pref}_m15_s2M.histo
$JF stats ${pref}_m15_s2M.jf.1.0 > ${pref}_m15.stats

# Count without size doubling
$JF mcount -t $nCPUs -o ${pref}_m15_s16M.jf -s 16M -C -m 15 seq10m.fa
$JF histo ${pref}_m15_s16M.jf.1.0 > ${pref}_m15_s16M.histo

# Count large merges in binary and text. Should agree
$JF mcount -m 40 -t $nCPUs -o ${pref}_text.jf -s 2M --text seq1m_0.fa
$JF mcount -m 40 -t $nCPUs -o ${pref}_binary.jf -s 2M seq1m_0.fa
$JF histo ${pref}_text.jf.1.0 > ${pref}_text.histo
$JF stats ${pref}_text.jf.1.0 > ${pref}_text.stats
$JF histo ${pref}_binary.jf.1.0 > ${pref}_binary.histo
$JF stats ${pref}_binary.jf.1.0 > ${pref}_binary.stats
$JF dump -c ${pref}_text.jf.1.0 | sort > ${pref}_text.dump
$JF dump -c ${pref}_binary.jf.1.0 | sort > ${pref}_binary.dump

# Check the lower and upper count without merging
$JF mcount -t $nCPUs -o ${pref}_m15_s2M_L2_U3.jf -s 2M -C -m 15 -L2 -U3 seq10m.fa
$JF histo ${pref}_m15_s2M_L2_U3.jf.1.0 > ${pref}_m15_s2M_L2_U3.histo
...
# Check query
$JF query ${pref}_binary.jf.1.0 -s seq1m_0.fa    | grep ' 1$' | wc -l | sed -e 's/ //g' > ${pref}_query_one_count
